...
| v1.6.0 | Audit fix pass: `encode_string`/`encode_bytes`/`build_publish_pkt` 16-bit length-prefix truncation guards, CI ASIL-B strict gate no longer masked with `|| true`, QoS-3 (both-bits-set) PUBLISH now closes the connection per MQTT-3.3.1-4 instead of silently downgrading to QoS 0, `Error::PayloadTooLarge` now actually produced by `publish()` instead of panicking, `incoming_qos2` receive map bounded against unbounded-memory DoS |
