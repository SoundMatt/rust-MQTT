...
| Family | Count | Scope |
|---|---|---|
| REQ-MQTT | 8 | Core library: spec version, client trait, lifecycle |
| REQ-MSG | 5 | MQTT message structure |
| REQ-V5-MSG | 5 | MQTT v5 message properties |
| REQ-QOS | 4 | Quality of Service levels |
| REQ-PUB | 6 | Publish path |
| REQ-SUB | 8 | Subscribe path |
| REQ-WILD | 8 | Topic wildcard matching (§4.7) |
| REQ-CONN | 11 | Connection lifecycle |
| REQ-CONC | 3 | Concurrency |
| REQ-MOCK | 5 | In-process mock broker |
| REQ-BROKER | 3 | Embedded TCP broker |
| REQ-RELAY | 20 | RELAY spec v1.10 conformance |
...
|---|---|---|
| Requirements management | Machine-readable registry + source annotations | `requirements.json` |
| Design | FMEA, TARA, module decomposition | `fmea.json`, `tara.json` |
| Implementation | Rust (safe subset), no `unsafe` blocks | Source code |
| Unit testing | `cargo test --locked` (70+ tests) | CI artifact |
...
| Supply-chain | `cargo audit`, SBOM, build provenance | `sbom.json`, `provenance.json` |
| RELAY conformance | `relay conform --strict`, `relay interop` | CI relay-conform job |
| Cybersecurity | `rsfusa cyber`, TARA | `cyber-report.json`, `tara.json` |
